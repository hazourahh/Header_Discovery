...
public class OOPH_LSH_test {

        public static void main(String[] args) {
            int lazo_k=1;
           MinHashLSH index = new MinHashLSH(0,lazo_k);
            {
                    ...
                    if (!wt.has_missing_header_line()) {
                        for (int i = 0; i < wt.getNumCols(); i++) {
                            ...
                            if (column_value.size() != 0) {
                                MinHashOptimal sketch = new  MinHashOptimal(lazo_k);
                ...
                test_set.parallel().forEach(
                        json_table ->
                        {
                            ...
                            for (int i = 0; i < w_table.getNumCols(); i++) {
                                ...
                                if (column_value.size() != 0) {
                                   MinHashOptimal sketch = new  MinHashOptimal(lazo_k);
                                    ...
                                    Set<Object> candidates = index.query(sketch);
                                    int h=10;
                                    for (Object current : candidates)
                                        if ( current != null &&
                                                !StringUtils.isBlank((String) current) &&
                                                ((String) current).length() <= 25  &&
                                                !((String) current).matches("[0-9]+")
                                        ) {
                                            results.add_candidate(i, new Header_Candidate(((String) current).replace(";", ""), h));
                                            h--;
                                        }
                                }
                            }
                           ...
                           // write_to_disk_runtime(w_table, w_table.getHeaders(), topk_time, coherance_time, topk_time + coherance_time);
                            ...
                            if(empty)
                                write_to_disk(w_table, w_table.getHeaders(), temp, -2, 1);
                            else {
                                for (int i = 0; i < results.getScored_candidates().length; i++) {
                                    if (results.getScored_candidates()[i].isEmpty()) {
                                        temp.add("NORESULT");
